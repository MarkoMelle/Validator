  ...
  if (arrValue[0] === 2) {
    if (arrValue.length === 16 && isSubmit) {
      ...
      response = {
        success: false,
        type: 'mir',
        message: 'Ошибка: ожидаемая длинна 16 символов'
      };
    } else {
      response = 'mir';
    }
    ...
    if (arrValue[1] === 7 || arrValue[1] === 4) {
      if (arrValue.length === 15 && isSubmit) {
        ...
        response = {
          success: false,
          type: 'amex',
          message: 'Ошибка: ожидаемая длинна 15 символов'
        };
      } else {
        response = 'amex';
      }
      ...
      if (arrValue.length === 15 && isSubmit) {
        ...
        response = {
          success: false,
          type: 'diners_club',
          message: 'Ошибка: ожидаемая длинна 14 символов'
        };
      } else {
...
var InnFormWidget = /*#__PURE__*/function () {
  function InnFormWidget(container) {
    _classCallCheck(this, InnFormWidget);
    this.container = container;
    this.onSubmit = this.onSubmit.bind(this);
    this.onInput = this.onInput.bind(this);
  }
  _createClass(InnFormWidget, [{
    key: "bindToDOM",
    ...
    key: "iconToggle",
    value: function iconToggle(value) {
      this.container.querySelectorAll('.card').forEach(function (e) {
    ...
    value: function onSubmit(e) {
      ...
      var response = getCardType(value, true);
      if (response.success) {
        this.successImg.classList.remove('hidden');
        this.iconToggle(response.type);
        var width = this.form.clientWidth;
        this.successImg.style = "left:".concat(1 * width + 15, "px;");
        ...
        var _width = this.input.clientWidth;
        this.tooltip.style = "top:".concat(1.1 * height, "px; width:").concat(_width, "px ");
    ...
    value: function onInput(e) {
      ...
      this.timeout = setTimeout(function () {
        return _this.iconToggle(getCardType(value, false));
      }, 300);
    }
  }]);
  return InnFormWidget;
}();

;// CONCATENATED MODULE: ./src/js/app.js

var container = document.querySelector('.widget-container');
var widget = new InnFormWidget(container);
widget.bindToDOM();
